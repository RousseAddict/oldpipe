...
class VideoPlayerVC: UIViewController, UIActionSheetDelegate, UIAlertViewDelegate {
    ...
    private func loadStreams(updateStatus: Bool = true) {
        ...
        YoutubeAPI.getStreams(videoId: video.id) { [weak self] streams, _, desc in
            ...
            DebugLog.log("Player", "getStreams result id=\(self.video.id) formats=\(streams.count) itags=\(streams.map { $0.itag })")
            if updateStatus {
    ...
    @objc private func playTapped() {
        ...
        if iosVersion >= 7.0 {
            ...
            DebugLog.log("Player", "id=\(video.id) StreamProxy.localURL failed — going straight to download")
    ...
    private func startPlayback(url: URL, isLocal: Bool) {
        ...
        pollUntilReady(maxTicks: 40, interval: 0.25, onReady: { [weak self] in
            ...
            DebugLog.log("Player", "startPlayback FAILED id=\(self.video.id) isFailed=\(self.sp.isFailed) itemError=\(itemError)")
            self.hideSpinner()
    ...
    private func tryStream(urlStr: String, fallbackDownload: String, maxTicks: Int = 16) {
        ...
        pollUntilReady(maxTicks: maxTicks, interval: 0.25, onReady: { [weak self] in
            ...
            guard let self = self else { return }
            // isFailed vs timeout are distinguishable here (before abandonLoad tears down the
            // item) — the AVPlayerItem error, when present, is the single most useful clue for
            // "video won't play" since AVPlayer failures otherwise surface nowhere in the UI.
            let itemError = self.sp.currentItemError?.localizedDescription ?? "none"
            DebugLog.log("Player", "tryStream FAILED id=\(self.video.id) url=\(urlStr) isFailed=\(self.sp.isFailed) itemError=\(itemError) — falling back to download")
